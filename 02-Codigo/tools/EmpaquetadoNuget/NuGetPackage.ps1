...
function Publish {
	...
	# Get nuget config
	[xml]$nugetConfig = Get-Content 02-Codigo\tools\EmpaquetadorNuget\NuGet.Config
	...
	$nugetConfig.configuration.config.add | ForEach-Object {
		...
		Get-ChildItem *.nupkg | Where-Object { $_.Name.EndsWith(".symbols.nupkg") -eq $false } | ForEach-Object { 
         if ($ambiente_conf=$Ambiente){
			# Try to push package
			$task = Create-Process 02-Codigo\tools\EmpaquetadorNuget\NuGet.exe ("push " + $_.Name + " -s " + $url + " " + $apikey + " -Verbosity Detailed")
			$task.Start() | Out-Null
			$task.WaitForExit()
...
Set-ItemProperty 02-Codigo\tools\EmpaquetadorNuget\NuGet.exe -Name IsReadOnly -Value $false
...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process 02-Codigo\tools\EmpaquetadorNuget\NuGet.exe ("pack Package.nuspec -Symbol -Verbosity Detailed")
...
Else {
	$packageTask = Create-Process 02-Codigo\tools\EmpaquetadorNuget\NuGet.exe ("pack Package.nuspec -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
