...
function HandlePublishError {
	...
	$setupTask = Start-Process PowerShell.exe "-ExecutionPolicy Unrestricted -File .\EmpaquetadoNuget\NuGetSetup.ps1 -Url $url -Base64EncodedMessage $encodedMessage" -Wait -PassThru
...
function Publish {
	...
	# Get nuget config
	[xml]$nugetConfig = Get-Content .\EmpaquetadoNuget\NuGet.Config
...
#Set-ItemProperty 'C:\Program Files (x86)\NuGet\NuGet.exe' -Name IsReadOnly -Value $false
...
Write-Log (Invoke-Command {& 'C:\Program Files (x86)\NuGet\NuGet.exe' update -Self} -ErrorAction Stop)
